// The gomock example suite, ported verb for verb onto minimock: the check that
// the vocabulary shape does not change when the mocking library does. The verbs
// arrange through the mock's own type and the subject receives it through the
// interface. minimock has no mid-test "are we satisfied" call — an unmet
// expectation is reported by the controller when the test finishes — so the
// capture-and-inspect pattern below is the way to put an interaction failure on
// the test's own line.
package mokkitminimock_test
...
import (
	...
	"github.com/GrafGenerator/go-mokkit/container/mokkitminimock"
...
// With minimock the interaction assertion is declared here rather than asserted
// later in Inspect — the controller checks it when the test finishes.
//
...
func (a Arrange) UserExists[K mokkit.Token[shop.User]](opts ...userOpt) Arrange {
	...
	a.Add("UserExists["+mokkit.NameOf[K]()+"]", func(_ context.Context, h mokkit.Host) error {
		...
		h.Resolve[*shop.UserRepositoryMock]().ByIDMock.
			When(minimock.AnyContext, u.ID).Then(u, nil)
...
func (a Arrange) DiscountRateIs(status string, rate float64) Arrange {
	...
	a.Add("DiscountRateIs", func(_ context.Context, h mokkit.Host) error {
		h.Resolve[*shop.RateRepositoryMock]().RateForMock.
			When(minimock.AnyContext, status).Then(rate, nil)
...
func (a Arrange) AuditAccepts() Arrange {
	...
	a.Add("AuditAccepts", func(_ context.Context, h mokkit.Host) error {
		h.Resolve[*shop.AuditMock]().RecordMock.
			Set(func(context.Context, string, float64) error { return nil })
...
func (a Arrange) AuditRecords[K mokkit.Token[recordedAudit]]() Arrange {
	...
	a.Add("AuditRecords["+mokkit.NameOf[K]()+"]", func(_ context.Context, h mokkit.Host) error {
		...
		h.Resolve[*shop.AuditMock]().RecordMock.
			Set(func(_ context.Context, userID string, amount float64) error {
...
func newFixture(t *testing.T) *fixture {
	...
	mokkitminimock.Add[shop.UserRepository](mocks, shop.NewUserRepositoryMock)
	mokkitminimock.Add[shop.RateRepository](mocks, shop.NewRateRepositoryMock)
	mokkitminimock.Add[shop.Audit](mocks, shop.NewAuditMock)
...
func TestCalculateDiscount_ForVipUser_AppliesTieredRate(t *testing.T) {
	...
	// Note what is absent: no UserRepositoryQueried verb. The Expect declared
	// by AUser is the interaction assertion; the controller reports it when
	// this test finishes.
}
