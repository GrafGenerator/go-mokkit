// A suite written the way the docs say to write one, but over generated mocks
// instead of hand-rolled fakes. It is the check that the vocabulary shape does
// not change when the doubles do: the verbs below arrange through the mock's
// own type, the subject receives it through the interface, and Satisfied()
// pulls an unmet expectation forward to the test's own line.
...
// exactly once. With gomock the interaction assertion is declared here rather
// than asserted later in Inspect — the controller checks it when the test
...
func (a Arrange) UserExists[K mokkit.Token[shop.User]](opts ...userOpt) Arrange {
	...
	a.Add("UserExists["+mokkit.NameOf[K]()+"]", func(_ context.Context, h mokkit.Host) error {
		...
		h.Resolve[*shop.MockUserRepository]().EXPECT().
			ByID(gomock.Any(), u.ID).Return(u, nil).Times(1)
...
func (a Arrange) DiscountRateIs(status string, rate float64) Arrange {
	...
	a.Add("DiscountRateIs", func(_ context.Context, h mokkit.Host) error {
		h.Resolve[*shop.MockRateRepository]().EXPECT().
			RateFor(gomock.Any(), status).Return(rate, nil).AnyTimes()
...
func (a Arrange) AuditAccepts() Arrange {
	...
	a.Add("AuditAccepts", func(_ context.Context, h mokkit.Host) error {
		h.Resolve[*shop.MockAudit]().EXPECT().
			Record(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
...
func (a Arrange) AuditRecords[K mokkit.Token[recordedAudit]]() Arrange {
	...
	a.Add("AuditRecords["+mokkit.NameOf[K]()+"]", func(_ context.Context, h mokkit.Host) error {
		...
		h.Resolve[*shop.MockAudit]().EXPECT().
			...
			DoAndReturn(func(_ context.Context, userID string, amount float64) error {
...
func TestCalculateDiscount_ForVipUser_AppliesTieredRate(t *testing.T) {
	...
	f.Inspect().
		...
		And(mokkitgomock.Satisfied())

	// Note what is absent: no UserRepositoryQueried verb. The Times(1) declared
